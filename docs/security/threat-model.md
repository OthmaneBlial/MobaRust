...
| Clipboard exposure | Paste is explicit; MobaRust intercepts multiline terminal paste and asks for confirmation before sending it. RDP clipboard is disabled by default and opt-in per profile; the current native backend is Windows-only, while macOS/Linux reject the request until reviewed adapters exist. Remote content is never copied into the Mac clipboard automatically. |
