...
| Private-key exposure | Keys are referenced, not copied into session metadata. Zeroization and platform-backed storage are required before key auth ships. |
...
| Compromised application database | Keep secrets separate from ordinary state; validate imported data; add platform vault or an audited encrypted portable vault. |
