...
to terminate when its Rust child handle is dropped. The traceroute output is
treated as untrusted diagnostic text and is truncated to a bounded number and
length of lines before it crosses the Tauri event boundary. React renders it
as escaped preformatted text rather than HTML.
...
- traceroute output is bounded before IPC and display;
