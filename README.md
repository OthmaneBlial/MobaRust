...
| **VNC** | Native helper with local RFB fixtures, authentication, raw/copy/Tight-JPEG framebuffer updates, bounded keyboard/mouse input with Unicode-to-X11 keysym mapping, explicit clipboard opt-in, scaling, quality profiles, bounded reconnect, cancellation, clean shutdown, and visible unencrypted-transport capability reporting; remote hostname/IP targets are available only through an explicit unencrypted-TCP opt-in, while TLS-capable engines and broader interoperability remain open |
