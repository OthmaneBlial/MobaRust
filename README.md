...
- RDP/VNC helper-pipe writes have a dedicated deadline; VNC protocol input and refresh writes also have a two-second bound, while closed RDP input channels become a bounded failure/reconnect state instead of an unhandled process error. Failures become one redacted crash diagnostic, and reader exits trigger bounded helper cleanup/reaping.
