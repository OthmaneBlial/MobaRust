...
impl RdpServerDisplay for FixtureDisplay {
    ...
    async fn updates(&mut self) -> anyhow::Result<Box<dyn RdpServerDisplayUpdates>> {
        let updates = self
            .updates
            .take()
            .expect("fixture display was initialized twice");
...
fn fixture_bitmap() -> BitmapUpdate {
...
async fn real_helper_controls_a_real_loopback_rdp_server() {
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            display_tx
                .send(DisplayUpdate::Bitmap(fixture_bitmap()))
                .await
