...
const GATEWAY_FIXTURE_SECRET: &str = "fixture-gateway-process-secret";
...
#[tokio::test]
async fn real_helper_waits_for_the_gateway_credential_before_starting() {
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
        .await
        .expect("could not reserve a disposable gateway loopback port");
    let gateway_port = listener
        .local_addr()
        .expect("disposable gateway listener had no address")
        .port();
    let accept_task = tokio::spawn(async move {
        if let Ok((socket, _)) = listener.accept().await {
            drop(socket);
    ...
    send_frame(
        &mut stdin,
        &encode_credential_frame(&HelperCredential::new(FIXTURE_SECRET))
            .expect("could not encode RDP session credential frame"),
    )
    .await;
    send_frame(
        &mut stdin,
        &encode_credential_frame(&HelperCredential::gateway(GATEWAY_FIXTURE_SECRET))
            .expect("could not encode RDP gateway credential frame"),
    )
    .await;
    stdin
        .flush()
    ...
    assert!(matches!(
        next_event(&mut stdout, "gateway ready").await,
        HelperEvent::State {
            state: HelperState::Ready
        }
    ));
    ...
    let mut terminal_state = None;
    for _ in 0..3 {
        match next_event(&mut stdout, "gateway failure").await {
            HelperEvent::Diagnostic { message, .. } => {
                assert!(!message.contains(FIXTURE_SECRET));
                assert!(!message.contains(GATEWAY_FIXTURE_SECRET));
            }
            HelperEvent::State {
                state: state @ (HelperState::Failed | HelperState::Stopped | HelperState::Crashed),
            } => {
    ...
    assert!(
        terminal_state.is_some(),
        "RDP helper did not report a terminal Gateway state"
    );

    let status = timeout(Duration::from_secs(5), child.wait())
        .await
        .expect("RDP helper did not exit after the closed Gateway fixture")
        .expect("could not wait for RDP helper");
    accept_task.abort();
    let _ = accept_task.await;
    assert!(
